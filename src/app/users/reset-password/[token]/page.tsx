    ...
    <div className="mx-auto mt-10 w-full max-w-[375px] rounded-lg bg-white p-6 shadow-lg">
      ...
      <div className="relative mb-4">
        ...
        <button
          ...
          className={clsx("absolute top-9 right-3 text-gray-600 hover:text-gray-800", { hidden: !password })}>
          {showPassword ? <GoEye size={25} /> : <PiEyeClosed size={25} />}
        </button>

        {!isPasswordValid && password && <p className="mt-1 mb-3 text-red-500">영문 포함하여 6자리 이상이어야 합니다.</p>}
      </div>
      ...
      <div className="relative mb-3">
        ...
        <button
          ...
          className={clsx("absolute top-2 right-3 text-gray-600 hover:text-gray-800", { hidden: !confirmPassword })}>
          {showConfirmPassword ? <GoEye size={25} /> : <PiEyeClosed size={25} />}
        </button>

        <div className={clsx("", { hidden: !password || !confirmPassword })}>
          {!isPasswordMatch ? (
            <p className="mt-1 mb-3 animate-pulse text-red-500">비밀번호가 일치하지 않습니다.</p>
          ) : (
            <p className="mt-1 mb-3 text-green-500">비밀번호가 일치합니다.</p>
          )}
        </div>
      </div>
      ...
      <div className={"relative"}>
        ...
        {resetPassword.isPending && <AiOutlineLoading3Quarters className={"absolute top-3.5 left-10 animate-spin md:left-12"} />}
      </div>
